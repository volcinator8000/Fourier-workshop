...
def get_signals(freq1, freq2):
    sig1 = np.cos(2 * np.pi * freq1 * t)
    sig2 = np.cos(2 * np.pi * freq2 * t)
    return sig1, sig2, sig1 + sig2

sig1, sig2, signal = get_signals(f1, f2)
...
ax_box1 = plt.axes([0.15, 0.90, 0.15, 0.04])
